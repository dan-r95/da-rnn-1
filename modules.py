...
class Encoder(nn.Module):
    ...
    def __init__(self, input_size: int, hidden_size: int, T: int):
        ...
        self.attn_linear = nn.Linear(
            in_features=2 * hidden_size + T - 1, out_features=1)

    def forward(self, input_data):
        # input_data: (batch_size, T - 1, input_size)
        input_weighted = Variable(torch.zeros(
            input_data.size(0), self.T - 1, self.input_size)).to(device)
        input_encoded = Variable(torch.zeros(
            input_data.size(0), self.T - 1, self.hidden_size)).to(device)
...
class Decoder(nn.Module):
    ...
    def __init__(self, encoder_hidden_size: int, decoder_hidden_size: int, T: int, out_feats=1):
        ...
        self.lstm_layer = nn.LSTM(
            input_size=out_feats, hidden_size=decoder_hidden_size)
        self.fc = nn.Linear(encoder_hidden_size + out_feats, out_feats)
        self.fc_final = nn.Linear(
            decoder_hidden_size + encoder_hidden_size, out_feats)
    ...
    def forward(self, input_encoded, y_history):
        ...
        context = Variable(torch.zeros(
            input_encoded.size(0), self.encoder_hidden_size)).to(device)
